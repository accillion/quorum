...
**Status:** v0.2 draft. Adjudicated peer-review pass applied. Open for Rolf review → commit and re-review session.
...
Items v0.2 leaves unresolved or partially deferred; v1.0 (post-implementation) should close these. Q9 closed in v0.2 (see §4.1); Q11 closed in v0.2 (see §A1 / §11); both kept as closed notes below for traceability.
...
- **Q9 — Migration rollback path.** *Resolved in v0.2; see §4.1.* The `schema_meta.forward_compat_min_version = '2'` row + the binary-side check on every open rejects a v3+ DB opened by a v0.2 binary with a clear upgrade message.
...
- **Q11 — `--force` / Shift+P bypass of the recurrence gate.** *Removed in v0.2 per peer-review consensus (3-of-3 against shipping). Future phase may reconsider with a fresh adjudication if user demand emerges. See §11 Phase 1C v2 list.*
