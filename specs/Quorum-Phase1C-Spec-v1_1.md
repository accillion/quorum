...
**Status:** v1.1 — Phase 1C implementation complete. Supersedes v1.0 (the implementation target) with three micro-revisions reconciling spec wording to as-built reality: §4.2 / §4.3 `finding_identity_hash` column type, §4.3 `convention_text` storage when body input is empty, §10 Q15 implementation note. Implementation lifecycle and lineage are in `HISTORY.md` Phase 1C.
...
CREATE TABLE state_transitions (
  ...
  finding_identity_hash TEXT NOT NULL REFERENCES dismissals(finding_identity_hash) ON DELETE CASCADE,
...
- **Column type is TEXT, not BLOB** (v1.1 reconciliation). `dismissals.finding_identity_hash` (Phase 1B v1) is TEXT — SHA-256 stored as 64-char hex. SQLite FK enforcement requires matching column affinity, so the new tables inherit TEXT. Byte-count semantics for CHECK constraints are unaffected (the CHECK on `conventions.convention_text` casts to BLOB explicitly; see §4.3).
...
CREATE TABLE conventions (
  finding_identity_hash TEXT PRIMARY KEY REFERENCES dismissals(finding_identity_hash) ON DELETE CASCADE,
...
- **`convention_text` when body input is empty** (v1.1 clarification). When `--text` / `--from-editor` / the TUI modal supplies no body, the rendered block in conventions.md carries only the title-derived header line (per AC 139). The SQLite `convention_text` column, constrained by the ≥1-byte CHECK, stores the dismissal title verbatim in this case. The block-in-file remains title-only; only the SQLite storage falls back to title. The dismissal `note` and `body_snapshot` are still never read (§4.6 leak surface).
- **`finding_identity_hash` column type is TEXT, not BLOB** (v1.1 reconciliation). Same rationale as §4.2.
...
- **Q15 — Promote on dirty conventions.md.** If the user runs `quorum convention promote <hash>` while conventions.md has uncommitted modifications outside the managed section, the temp-file-rename pattern preserves the user's edits (the temp file is built from the working-tree contents plus the new block, then renamed). But should the CLI warn? Or refuse if the modifications are *inside* the managed section (where Quorum is about to overwrite)? Lean: warn-but-proceed on outside-fence modifications; warn-and-confirm (`Y/N` prompt) on inside-fence modifications detected by the orphan-check pass.
  ...
  **v1.1 implementation note (Phase 1C Stage 4):** v1.0 implementation simplifies to warn-on-any-pre-flight-discrepancy on both CLI promote/demote paths. Inside-vs-outside-fence detection is deferred — see `BACKLOG.md`. The TUI promote/demote path runs no pre-flight diagnostics at all (also deferred — `BACKLOG.md`).
